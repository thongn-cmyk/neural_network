...
namespace matrix_deviation_calculator
{
    ...
    template <class PromotedFloatType = tensor_std_float_t, class ScoreCalculator>
    class GenericDeviationCalculator: public virtual MatrixDeviationCalculatorInterface
    {
        private:
        ...
        public:
            ...
            GenericDeviationCalculator(ScoreCalculator score_calculator,
                                       size_t group_sz): score_calculator(std::move(score_calculator)),
                                                         group_sz(group_sz)
            {
                if (group_sz == 0u)
                {
        ...
        private:
            ...
            {
                PromotedFloatType sum_difference = 0;
                ...
                size_t revolution_sz    = lhs_logit_vec.size() / this->group_sz + static_cast<size_t>(lhs_logit_vec.size() % this->group_sz != 0u);

                for (size_t i = 0u; i < revolution_sz; ++i)
                {
                    size_t first                    = i * this->group_sz;
                    size_t last                     = std::min(static_cast<size_t>((i + 1) * this->group_sz), lhs_logit_vec.size());
                    auto lhs_span                   = std::span<tensor_std_float_t>(std::next(lhs_logit_vec.begin(), first), last - first);
                    auto rhs_span                   = std::span<tensor_std_float_t>(std::next(rhs_logit_vec.begin(), first), last - first);

                    PromotedFloatType difference    = this->score_calculator(lhs_span, rhs_span);
                ...
                return sum_difference / stdx::safe_non_zero_access(revolution_sz);
            }
    };
