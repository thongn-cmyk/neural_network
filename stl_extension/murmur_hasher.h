#ifndef __STL_EXTENSION_MURMUR_HASHER_H__
#define __STL_EXTENSION_MURMUR_HASHER_H__
...
namespace murmur_hasher
{
    ...
    static constexpr auto murmur_hash_base(const char * buf, size_t len, const uint32_t seed = 0xFF) -> std::pair<uint64_t, uint64_t>{
        ...
        {   
            ...
            trivial_serializer::deserialize_into(k1, buf + (i * 2 + 0) * sizeof(uint64_t));
            trivial_serializer::deserialize_into(k2, buf + (i * 2 + 1) * sizeof(uint64_t));
    ...
    static constexpr auto murmur_hash_base(const char * buf, const std::integral_constant<size_t, LEN>, const std::integral_constant<size_t, SEED> seed = std::integral_constant<size_t, SEED>{}) -> std::pair<uint64_t, uint64_t>
    {
        const size_t nblocks = LEN / 16;
    ...
    template <size_t LEN, size_t SEED = 0xFF>
    constexpr auto murmur_hash(const char * buf, const std::integral_constant<size_t, LEN> len, const std::integral_constant<size_t, SEED> seed) -> uint64_t
    {
        return std::get<0>(murmur_hash_base(buf, len, seed));
    }
}
