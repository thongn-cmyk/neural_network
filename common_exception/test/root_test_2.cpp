...
auto run_one_root_test() -> double
{
    ...
    static std::unique_ptr<global_optimality_approximator::TensorFactoryInterface> tensor_factory = global_optimality_approximator::TensorFactoryFactory::get_best_factory<long double>();

    RootEquation root_eqn(root_vec);

    for (size_t i = 0u; i < TEST_SZ; ++i)
    {
        auto tensor = tensor_factory->get();
        std::shared_ptr<global_optimality_approximator::TimeMachineOptimizerInterface> optimizer = tensor->get();
...
auto run_one_root_offset_test() -> double
{
    ...
    double offset                               = randomize_real() * 100000ULL + 10000;
