#ifndef __DG_GLOBAL_ALLOCATOR_H__
#define __DG_GLOBAL_ALLOCATOR_H__
...
namespace allocation_base::global_allocator
{
    template <class Key, class Value>
    using unordered_map         = datastructure::unordered_map_variants::unordered_node_map<Key, Value>;

    template <class Value>
    using pow2_cyclic_queue     = datastructure::queue::pow2_cyclic_queue<Value>;
    ...
    class BinaryUnitAllocator
    {
        ...
        private:
            ...
            static_assert(stdx::is_pow2(DEFAULT_ALIGNMENT_SZ));
            static inline constexpr size_t MIN_POW2_VALUE       = std::max(size_t{5u}, static_cast<size_t>(stdx::ulog2(DEFAULT_ALIGNMENT_SZ)));
    ...
    class DistributedThreadSafeBatchBinaryUnitAllocator
    {
        ...
        public:
            ...
            static_assert(stdx::is_pow2(CONCURRENCY_SZ));
            ...
            inline void malloc(size_t * sz_arr, size_t sz, std::add_pointer_t<void> * result_arr)
            {
                ...
                size_t mem_sz                   = funnel::delvrsrv_kv_allocation_cost(&feed_resolutor, trimmed_feed_sz);
                allocation_base::stack_allocator::NoExceptRawAllocation<char[]> mem(mem_sz);
                auto feeder                     = common_exception::remove_expected(funnel::delvrsrv_kv_open_preallocated_raiihandle(&feed_resolutor, trimmed_feed_sz, mem.get()));
                ...
                for (size_t i = 0u; i < sz; ++i)
                {
                    ...
                    {
                        uint8_t idx         = affined_randomizer::randomize_int<uint8_t>() & (CONCURRENCY_SZ - 1u);
                        MallocArgument arg  =
                        {
                            .sz     = sz_arr[i],
                        ...
                        funnel::delvrsrv_kv_deliver(feeder.get(), idx, arg);
                    }
                }
            }
            ...
            inline void free(std::add_pointer_t<void> * buf_arr, size_t sz) noexcept
            {
                ...
                for (size_t i = 0u; i < sz; ++i)
                {
                    ...
                    else [[likely]]
                    {
                        ...
                        funnel::delvrsrv_kv_deliver(feeder.get(), idx, static_cast<void *>(buf_head));
                    }
                }
            }
        ...
        private:
            ...
            struct MallocFeedResolutor: funnel::KVConsumerInterface<uint8_t, MallocArgument>
            {
                std::vector<std::unique_ptr<ThreadSafeBatchBinaryUnitAllocator>> * allocator_vec;

                void push(const uint8_t& idx, std::move_iterator<MallocArgument *> malloc_argument_arr, size_t sz) noexcept
                {
                    allocation_base::stack_allocator::NoExceptAllocation<std::add_pointer_t<void>[]> mem_vec(sz);
                    allocation_base::stack_allocator::NoExceptAllocation<size_t[]> mem_sz_vec(sz);
            ...
            struct FreeFeedResolutor: funnel::KVConsumerInterface<uint8_t, std::add_pointer_t<void>>
            {
                std::vector<std::unique_ptr<ThreadSafeBatchBinaryUnitAllocator>> * allocator_vec;
    ...
    class AffinedAllocator
    {
        ...
        public:
            ...
            AffinedAllocator(std::shared_ptr<BestBinaryUnitAllocator> base_allocator)
            {
                ...
                {
                    pow2_cyclic_queue<void *> pool(stdx::ulog2(stdx::ceil2(STACK_CAPTURE_POOL_POPULATION)));
        ...
        private:
            ...
            __attribute__((noinline)) auto slow_malloc(size_t sz) -> void *
            {
                ...
                allocation_base::stack_allocator::NoExceptAllocation<size_t[]> sz_arr(REFILL_SZ);
                allocation_base::stack_allocator::NoExceptAllocation<std::add_pointer_t<void>[]> result_arr(REFILL_SZ);
    ...
    class RoundBucketAffinedAllocator: private LargeSmallAffinedAllocator
    {
        ...
        private:
            ...
            {
                return stdx::ceil2(sz + SLACK_BUFFER_SZ) - SLACK_BUFFER_SZ;
            }
    };
    ...
    using allocation_resource_obj = stdx::singleton_container<std::shared_ptr<GlobalAllocator>, AllocationResourceSignature>;
    ...
    auto get_allocator_instance() noexcept -> const std::shared_ptr<GlobalAllocator>&
    {
        if constexpr(DEBUG_MODE_FLAG)
        {
        ...
        return allocation_resource_obj::get();
    }
    ...
    {
        return get_allocator_instance()->malloc(blk_sz); 
    }

    extern void dg_free(void * ptr) noexcept
    {
        get_allocator_instance()->free(ptr);
    }
    ...
    extern auto dg_aligned_alloc(size_t alignment, size_t blk_sz) -> void *
    {
        ...
        void * ptr          = get_allocator_instance()->malloc(adj_blk_sz);
        ...
        void * aligned_ptr              = global_allocator::dg_align(std::next(static_cast<char *>(ptr), sizeof(alignment_header_t)), alignment);
    ...
    extern auto dg_xaligned_alloc(size_t alignment, size_t blk_sz) -> void *
    {
        ...
        void * aligned_ptr              = global_allocator::dg_align(std::next(static_cast<char *>(ptr), METADATA_SZ), alignment); //forward METADATA_SZ to reserve the METADATA_SZ, align the alignment (guaranteed to fit because we have extra ALIGMENT_SZ - 1u)
        alignment_header_t difference   = std::distance(static_cast<char *>(ptr), static_cast<char *>(aligned_ptr));
    ...
    auto std_new_object(Args&& ...args) -> T *
    {
        ...
        if constexpr(alignof(T) <= global_allocator::DEFAULT_ALIGNMENT_SZ)
        {
            blk = global_allocator::dg_malloc(sizeof(T));
        }
        else
        {
            blk = global_allocator::dg_aligned_alloc(alignof(T), sizeof(T));
        }
        ...
        else
        {
            ...
            catch (...)
            {
                if constexpr(alignof(T) <= global_allocator::DEFAULT_ALIGNMENT_SZ)
                {
                    global_allocator::dg_free(blk);
                }
                else
                {
                    global_allocator::dg_aligned_free(blk);
                }
    ...
    auto std_delete_object(T * obj) noexcept
    {
        ...
        if constexpr(alignof(T) <= global_allocator::DEFAULT_ALIGNMENT_SZ)
        {
            [[clang::noinline]] global_allocator::dg_free(static_cast<void *>(obj));
        }
        else
        {
            [[clang::noinline]] global_allocator::dg_aligned_free(static_cast<void *>(obj));
        }
    }
    ...
    auto std_new_array(size_t sz) -> T *
    {
        ...
        void * blk                  = global_allocator::dg_xaligned_alloc(alignof(T), allocation_blk_sz);
        ...
        else
        {
            ...
            {
                global_allocator::dg_xaligned_free(blk);
                throw;
            }
        }
    }
    ...
    void std_delete_array(T * arr) noexcept
    {
        ...
        size_t allocation_blk_sz    = global_allocator::dg_xaligned_blk_size(arr);
        ...
        [[clang::noinline]] global_allocator::dg_xaligned_free(static_cast<void *>(arr));
    }
    ...
    class NoExceptAllocator
    {
        private:
            ...
            {
                return alignof(T) <= global_allocator::DEFAULT_ALIGNMENT_SZ;
            }
        ...
        public:
            ...
            auto allocate(std::size_t n) -> pointer
            {
                ...
                {
                    return static_cast<pointer>(global_allocator::dg_malloc(sizeof(T) * n));
                }
                else
                {
                    return static_cast<pointer>(global_allocator::dg_aligned_alloc(alignof(T), sizeof(T) * n));
                }
            }
            ...
            void deallocate(T* p, std::size_t n) noexcept
            {
                if constexpr(is_no_align_malloc())
                {
                    [[clang::noinline]] global_allocator::dg_free(p);
                }
                else
                {
                    [[clang::noinline]] global_allocator::dg_aligned_free(p);
                }
            }
    };
