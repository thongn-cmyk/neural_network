...
class Allocator: public virtual immutable_memory::MemoryAllocatorInterface
{
    public:
...
class MemoryTester
{
    private:
        ...
        std::shared_ptr<immutable_memory::ExternalImmutableMemoryCacheInterface> mem_cache;
        size_t window_sz;
        size_t operation_sz;
    ...
    public:
        ...
        MemoryTester(std::shared_ptr<immutable_memory::ExternalImmutableMemoryCacheInterface> mem_cache,
                     size_t window_sz,
        ...
        void operator()()
        {
            for (size_t i = 0u; i < this->operation_sz; ++i)
            {
                ...
                if (!this->bucket_vec.empty())
                {
                    ...
                    if (randomizer() % 2 == 0u)
                    {
                        std::optional<immutable_memory::MemoryReference> mem_reference = this->mem_cache->acquire_memory(this->bucket_vec[idx].mem_ptr);
                    ...
                    {
                        immutable_memory::MemoryReference mem_reference = this->mem_cache->cache_n_acquire_memory(this->bucket_vec[idx].mem_ptr,
...
void run_one_immutable_memory_test()
{
    cron_subsystem::init();
    ...
    std::shared_ptr<immutable_memory::ExternalImmutableMemoryCacheInterface> memcache = immutable_memory::Factory::get_normal_immutable_memory_cache(std::chrono::duration_cast<std::chrono::nanoseconds>(std::chrono::milliseconds(randomize_uint(MIN_MEMORY_LIFETIME_IN_MILLISECONDS, MAX_MEMORY_LIFETIME_IN_MILLISECONDS))),
...
void run_immutable_memory_test()
{
    const size_t TEST_SZ    = size_t{1} << 20;
    const size_t COUT_SZ    = size_t{1} << 10;

    std::cout << "__BEGIN_IMMUTABLE_MEMORY_TEST__" << std::endl;

    for (size_t i = 0u; i < TEST_SZ; ++i)
    {
        run_one_immutable_memory_test();
    ...
    std::cout << "__END_IMMUTABLE_MEMORY_TEST__" << std::endl;
}

int main()
{
    run_immutable_memory_test();
}