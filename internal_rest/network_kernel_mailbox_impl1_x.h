...
namespace dg_sock::network_kernel_mailbox_impl1_flash_streamx{
    ...
    {
        dg_sock::vector<internal_huge_kernel_buffer> buffer_vec;
    ...
    static auto serialize_packet_segment(PacketSegment&& segment) noexcept -> std::expected<internal_segment_kernel_buffer, exception_t>{
        ...
        try{
            ...
            dg_sock::network_compact_trivial_serializer::serialize_into(std::next(segment.buf.data(), old_sz), header, PACKET_SEGMENT_SERIALIZATION_SECRET);
    ...
    static auto deserialize_packet_segment(internal_segment_kernel_buffer&& buf) noexcept -> std::expected<PacketSegment, exception_t>{
        ...
        if (buf_sz < header_sz){
            return std::unexpected(dg_sock::network_exception::SOCKET_STREAM_BAD_SEGMENT);           
        }

        size_t hdr_off      = buf.size() - header_sz;
        char * hdr_buf      = std::next(buf.data(), hdr_off);  

        exception_t err     = dg_sock::network_exception::to_cstyle_function(dg_sock::network_compact_trivial_serializer::deserialize_into<header_t>)(header, hdr_buf, header_sz, PACKET_SEGMENT_SERIALIZATION_SECRET);
    ...
    static auto internal_integrity_assembled_packet_to_buffer(AssembledPacket&& pkt) noexcept -> std::expected<internal_huge_kernel_buffer, exception_t>{
        ...
        if (has_mm_integrity_value){
            uint64_t integrity_value = dg_sock::network_hash::murmur_hash(rs->data(), rs->size(), PACKET_INTEGRITY_SECRET);
    ...
    class temporal_finite_unordered_map{
        ...
        private:
            ...
            dg_sock::cyclic_unordered_node_map<Key, Value> map_container;

        public:

            temporal_finite_unordered_map(size_t tentative_cap): map_container(dg_sock::cyclic_unordered_node_map<Key, Value>::size_to_capacity(tentative_cap)){}
    ...
    class temporal_finite_unordered_set
    {
        private:
            ...
            dg_sock::cyclic_unordered_node_set<Key> set_container;
    ...
    class ComplexReactor{
        ...
        private:
            ...
            dg_sock::unordered_unstable_map<std::thread::id, std::shared_ptr<dg_binary_semaphore>> mtx_queue;
        ...
        private:
            ...
            inline __attribute__((force_inline)) void do_release(dg_sock::unordered_unstable_map<std::thread::id, std::shared_ptr<dg_binary_semaphore>>& smp_vec){
    ...
    class RandomPacketIDGenerator: public virtual PacketIDGeneratorInterface{
        ...
        public:
            ...
            auto get_id() noexcept -> GlobalIdentifier{
                ...
                GlobalIdentifier id = {};
                id.addr             = this->factory_addr;
                id.local_id         = std::make_pair(dg_sock::network_randomizer::randomize_int<uint64_t>(), dg_sock::network_randomizer::randomize_int<uint64_t>()); //increase locality of packet_id to help with the memory pattern (maybe or maybe not)
    ...
    class TemporalAbsoluteTimeoutInBoundGate: public virtual InBoundGateInterface{
        ...
        private:
            ...
            std::unique_ptr<stdxx::fair_atomic_flag> mtx;
            stdxx::hdi_container<size_t> thru_sz_per_load;
        ...
        public:
            ...
            TemporalAbsoluteTimeoutInBoundGate(temporal_finite_unordered_map<GlobalIdentifier, std::chrono::time_point<std::chrono::steady_clock>> abstimeout_map,
                                               ...
                                               std::unique_ptr<stdxx::fair_atomic_flag> mtx,
                                               stdxx::hdi_container<size_t> thru_sz_per_load) noexcept: abstimeout_map(std::move(abstimeout_map)),
    ...
    class TemporalBlackListGate: public virtual BlackListGateInterface{
        ...
        public:
            ...
            TemporalBlackListGate(temporal_switching_bloom_filter<GlobalIdentifier> blacklist_set,
                                  std::unique_ptr<stdxx::fair_atomic_flag> mtx,
                                  stdxx::hdi_container<size_t> thru_sz_per_load) noexcept: blacklist_set(std::move(blacklist_set)),
                                                                                          mtx(std::move(mtx)),
                                                                                          thru_sz_per_load(thru_sz_per_load){}
    ...
    class RandomHashDistributedBlackListGate: public virtual BlackListGateInterface{
        ...
        public:
            ...
            void thru(GlobalIdentifier * global_id_arr, size_t sz, exception_t * exception_arr) noexcept{
                ...
                size_t trimmed_keyvalue_feed_cap    = std::min(this->keyvalue_feed_cap, sz);
                size_t thru_feeder_allocation_cost  = dg_sock::network_producer_consumer::delvrsrv_kv_allocation_cost(&thru_feed_resolutor, trimmed_keyvalue_feed_cap);
                dg_sock::network_stack_allocation::NoExceptRawAllocation<char[]> thru_feeder_mem(thru_feeder_allocation_cost);
                auto thru_feeder                    = dg_sock::network_exception_handler::nothrow_log(dg_sock::network_producer_consumer::delvrsrv_kv_open_preallocated_raiihandle(&thru_feed_resolutor, trimmed_keyvalue_feed_cap, thru_feeder_mem.get()));
                ...
                for (size_t i = 0u; i < sz; ++i){
                    ...
                    dg_sock::network_producer_consumer::delvrsrv_kv_deliver(thru_feeder.get(), partitioned_idx, thru_feed_arg);
                }
            }
            ...
            void blacklist(GlobalIdentifier * global_id_arr, size_t sz, exception_t * exception_arr) noexcept{
                ...
                size_t trimmed_keyvalue_feed_cap        = std::min(this->keyvalue_feed_cap, sz);
                size_t blacklist_feeder_allocation_cost = dg_sock::network_producer_consumer::delvrsrv_kv_allocation_cost(&blacklist_feed_resolutor, trimmed_keyvalue_feed_cap);
                dg_sock::network_stack_allocation::NoExceptRawAllocation<char[]> blacklist_feeder_mem(blacklist_feeder_allocation_cost);
                auto blacklist_feeder                   = dg_sock::network_exception_handler::nothrow_log(dg_sock::network_producer_consumer::delvrsrv_kv_open_preallocated_raiihandle(&blacklist_feed_resolutor, trimmed_keyvalue_feed_cap, blacklist_feeder_mem.get()));
                ...
                for (size_t i = 0u ; i < sz; ++i){
                    ...
                    dg_sock::network_producer_consumer::delvrsrv_kv_deliver(blacklist_feeder.get(), partitioned_idx, blacklist_feed_arg);
                }
            }
        ...
        private:
            ...
            struct InternalBlackListFeedResolutor: dg_sock::network_producer_consumer::KVConsumerInterface<size_t, InternalBlackListFeedArgument>{
    ...
    class Packetizer: public virtual PacketizerInterface{
        ...
        public:
            ...
            auto packetize(internal_segment_kernel_buffer&& buf) noexcept -> std::expected<internal_vector<PacketSegment>, exception_t>{
                ...
                if (buf.size() > this->pow2_max_stream_bsz){
                    return std::unexpected(dg_sock::network_exception::SOCKET_STREAM_BAD_BUFFER_LENGTH);
                }
                ...
                std::expected<internal_vector<PacketSegment>, exception_t> rs = dg_sock::network_exception::cstyle_initialize<internal_vector<PacketSegment>>(std::max(size_t{1}, segment_sz));
                ...
                for (size_t i = 0u; i < segment_sz; ++i){
                    ...
                    segment.has_mm_integrity_value                  = this->has_mm_integrity;
                    std::expected<internal_segment_kernel_buffer, exception_t> app_buf  = dg_sock::network_exception::cstyle_initialize<internal_segment_kernel_buffer>((last - first)); 
        ...
        private:
            ...
            auto integrity_encode(std::string_view buf, bool has_integrity_encode) noexcept -> uint64_t{
                ...
                return dg_sock::network_hash::murmur_hash(buf.data(), buf.size(), PACKET_INTEGRITY_SECRET);
            }
    };
    ...
    class PacketAssembler: public virtual PacketAssemblerInterface{
        ...
        private:
            ...
            dg_sock::cyclic_unordered_node_map<GlobalIdentifier, AssembledPacket> packet_map;
        ...
        public:
            ...
            PacketAssembler(dg_sock::cyclic_unordered_node_map<GlobalIdentifier, AssembledPacket> packet_map,
                            size_t max_segment_sz_per_stream,
                            std::unique_ptr<stdxx::fair_atomic_flag> mtx,
                            stdxx::hdi_container<size_t> consume_sz_per_load) noexcept: packet_map(std::move(packet_map)),
                ...
                for (size_t i = 0u; i < sz; ++i){
                    ...
                    if (map_ptr == this->packet_map.end()){
                        if (base_segment_arr[i].segment_sz > this->max_segment_sz_per_stream){
                            assembled_arr[i] = std::unexpected(dg_sock::network_exception::SOCKET_STREAM_BAD_SEGMENT_SIZE);
                            continue;
                        }
                    ...
                    if (map_ptr->second.collected_segment_sz == map_ptr->second.total_segment_sz){ 
                        ...
                        assembled_arr[i] = std::unexpected(dg_sock::network_exception::SOCKET_STREAM_SEGMENT_FILLING);
                    }
                }
            }
        ...
        private:
            ...
            static auto internal_packet_segment_put(AssembledPacket& pkt,
                ...
                if (pkt_segment.segment_idx >= pkt.data.size()){
                    return dg_sock::network_exception::INDEX_OUT_OF_RANGE;
                }
            ...
            auto make_empty_assembled_packet(size_t segment_sz,
                ...
                auto vec            = dg_sock::network_exception::cstyle_initialize<internal_vector<std::optional<internal_segment_kernel_buffer>>>(segment_sz);
    ...
    class RandomHashDistributedPacketAssembler: public virtual PacketAssemblerInterface{
        ...
        public:
            ...
            void assemble(std::move_iterator<PacketSegment *> segment_arr, size_t sz, std::expected<AssembledPacket, exception_t> * assembled_arr) noexcept{
                ...
                for (size_t i = 0u; i < sz; ++i){
                    size_t hashed_value             = dg_sock::network_hash::hash_reflectible(base_segment_arr[i].id);
                    size_t partitioned_idx          = hashed_value & (this->pow2_base_arr_sz - 1u);
            ...
            void destroy(GlobalIdentifier * id_arr, size_t sz) noexcept{
                ...
                for (size_t i = 0u; i < sz; ++i){
                    size_t hashed_value             = dg_sock::network_hash::hash_reflectible(id_arr[i]);
                    size_t partitioned_idx          = hashed_value & (this->pow2_base_arr_sz - 1u);

                    dg_sock::network_producer_consumer::delvrsrv_kv_deliver(feeder.get(), partitioned_idx, id_arr[i]);
                } 
            }
        ...
        private:
            ...
            struct InternalAssembleFeedResolutor: dg_sock::network_producer_consumer::KVConsumerInterface<size_t, InternalAssembleFeedArgument>{
                ...
                void push(const size_t& idx, std::move_iterator<InternalAssembleFeedArgument *> data_arr, size_t sz) noexcept{
                    ...
                    dg_sock::network_stack_allocation::NoExceptAllocation<PacketSegment[]> segment_arr(sz);
                    dg_sock::network_stack_allocation::NoExceptAllocation<std::expected<AssembledPacket, exception_t>[]> rs_arr(sz);
                    ...
                    for (size_t i = 0u; i < sz; ++i){
                        ...
                        if (rs_arr[i].error() == dg_sock::network_exception::SOCKET_STREAM_SEGMENT_FILLING){
                            *base_data_arr[i].rs = std::move(rs_arr[i]); //thru
                            continue;
                        }
            ...
            struct InternalDestroyFeedResolutor: dg_sock::network_producer_consumer::KVConsumerInterface<size_t, GlobalIdentifier>{
    ...
    class ExhaustionControlledPacketAssembler: public virtual PacketAssemblerInterface{
        ...
        public:
            ...
            void assemble(std::move_iterator<PacketSegment *> segment_arr, size_t sz, std::expected<AssembledPacket, exception_t> * assembled_arr) noexcept{
                ...
                size_t sliding_window_sz                                            = sz;
                auto full_signal                                                    = std::expected<AssembledPacket, exception_t>(std::unexpected(dg_sock::network_exception::QUEUE_FULL)); 

                auto task = [&, this]() noexcept{
                    this->base->assemble(std::make_move_iterator(first_segment_ptr), sliding_window_sz, first_assembled_ptr);
                    size_t queueing_sz  = std::count_if(first_assembled_ptr, last_assembled_ptr, [](const auto& e){return e.has_value() == false && e.error() == dg_sock::network_exception::QUEUE_FULL;});
                    exception_t err     = this->exhaustion_controller->update_waiting_size(queueing_sz);
                    ...
                    std::expected<AssembledPacket, exception_t> * first_retriable_ptr   = std::find_if(first_assembled_ptr, last_assembled_ptr, [](const auto& e){return e.has_value() == false && e.error() == dg_sock::network_exception::QUEUE_FULL;});
                    std::expected<AssembledPacket, exception_t> * last_retriable_ptr    = std::find_if(first_retriable_ptr, last_assembled_ptr, [](const auto& e){return e.has_value() == true || e.error() != dg_sock::network_exception::QUEUE_FULL;});
                ...
                auto virtual_task = dg_sock::network_concurrency_infretry_x::ExecutableWrapper<decltype(task)>(task);
                this->executor->exec(virtual_task);
            }
    ...
    class BufferFIFOContainer: public virtual InBoundContainerInterface{
        ...
        private:
            ...
            dg_sock::pow2_cyclic_queue<internal_huge_kernel_buffer> buffer_vec;
        ...
        public:
            ...
            void push(std::move_iterator<internal_huge_kernel_buffer *> buffer_arr, size_t sz, exception_t * exception_arr) noexcept{
                ...
                auto task = [&, this]() noexcept{
                    ...
                    size_t waiting_sz                   = std::count(exception_arr_first, exception_arr_last, dg_sock::network_exception::QUEUE_FULL);
                    ...
                    exception_t * retriable_arr_first   = std::find(exception_arr_first, exception_arr_last, dg_sock::network_exception::QUEUE_FULL);
                    exception_t * retriable_arr_last    = std::find_if(retriable_arr_first, exception_arr_last, [](exception_t err){return err != dg_sock::network_exception::QUEUE_FULL;});
    ...
    class FairInBoundBufferContainer: public virtual InBoundContainerInterface{
        ...
        private:
            ...
            dg_sock::pow2_cyclic_queue<dg_sock::vector<internal_huge_kernel_buffer>> distribution_queue;
            dg_sock::pow2_cyclic_queue<std::pair<std::optional<dg_sock::vector<internal_huge_kernel_buffer>> *, dg_binary_semaphore *>> waiting_queue;
            dg_sock::pow2_cyclic_queue<dg_sock::vector<internal_huge_kernel_buffer>> leftover_queue;
            std::unique_ptr<UnitAllocatorInterface<dg_sock::vector<internal_huge_kernel_buffer>>> bufvec_allocator;
            std::unique_ptr<stdxx::fair_atomic_flag> mtx;
            stdxx::hdi_container<size_t> consume_sz_per_load;
        ...
        public:
            ...
            void push(std::move_iterator<internal_huge_kernel_buffer *> buffer_arr, size_t sz, exception_t * exception_arr) noexcept{
                ...
                std::expected<dg_sock::vector<internal_huge_kernel_buffer>, exception_t> buffer_vec_tmp = this->bufvec_allocator->get();
                ...
                dg_sock::vector<internal_huge_kernel_buffer> buffer_vec = std::move(buffer_vec_tmp.value());
            ...
            void pop(internal_huge_kernel_buffer * output_buffer_arr, size_t& sz, size_t output_buffer_arr_cap) noexcept{
                ...
                std::optional<dg_sock::vector<internal_huge_kernel_buffer>> str_vec(std::nullopt); 
    ...
    class BufferContainerRedistributorWorker: public virtual dg_sock::network_concurrency::WorkerInterface{
        ...
        public:
            ...
            bool run_one_epoch() noexcept{
                ...
                dg_sock::network_stack_allocation::NoExceptAllocation<internal_huge_kernel_buffer[]> recv_buf_arr(this->fr_warehouse_get_cap);
        ...
        private:
            ...
            struct InternalDeliveryResolutor: dg_sock::network_producer_consumer::ConsumerInterface<internal_huge_kernel_buffer>{
    ...
    class ExpiryWorker: public virtual dg_sock::network_concurrency::WorkerInterface{
        ...
        public:
            ...
            bool run_one_epoch() noexcept{
                ...
                size_t id_arr_sz    = {};
                dg_sock::network_stack_allocation::NoExceptAllocation<GlobalIdentifier[]> id_arr(id_arr_cap);
                ...
                size_t pa_feeder_allocation_cost    = dg_sock::network_producer_consumer::delvrsrv_allocation_cost(&pa_feed_resolutor, trimmed_pa_feed_sz);
                dg_sock::network_stack_allocation::NoExceptRawAllocation<char[]> pa_feeder_mem(pa_feeder_allocation_cost);
                auto pa_feeder                      = dg_sock::network_exception_handler::nothrow_log(dg_sock::network_producer_consumer::delvrsrv_open_preallocated_raiihandle(&pa_feed_resolutor, trimmed_pa_feed_sz, pa_feeder_mem.get()));

                for (size_t i = 0u; i < id_arr_sz; ++i){
                    dg_sock::network_producer_consumer::delvrsrv_deliver(pa_feeder.get(), id_arr[i]);
                }
        ...
        private:
            ...
            struct InternalPacketAssemblerDestroyFeedResolutor: dg_sock::network_producer_consumer::ConsumerInterface<GlobalIdentifier>{
                ...
                void push(std::move_iterator<GlobalIdentifier *> id_arr, size_t id_arr_sz) noexcept{
                    ...
                    dg_sock::network_stack_allocation::NoExceptAllocation<exception_t[]> exception_arr(id_arr_sz);
    ...
    class InBoundWorker: public virtual dg_sock::network_concurrency::WorkerInterface{
        ...
        private:
            ...
            std::shared_ptr<EntranceControllerInterface> entrance_controller;
            std::shared_ptr<dg_sock::network_concurrency_infretry_x::ExecutorInterface> infretry_device;
        ...
        public:
            ...
            bool run_one_epoch() noexcept{
                ...
                dg_sock::network_stack_allocation::NoExceptAllocation<internal_segment_kernel_buffer[]> buf_arr(consuming_cap);
                ...
                size_t ib_feeder_allocation_cost            = dg_sock::network_producer_consumer::delvrsrv_allocation_cost(&ib_feed_resolutor, trimmed_ib_feed_cap);
                dg_sock::network_stack_allocation::NoExceptRawAllocation<char[]> ib_feeder_mem(ib_feeder_allocation_cost);
                auto ib_feeder                              = dg_sock::network_exception_handler::nothrow_log(dg_sock::network_producer_consumer::delvrsrv_open_preallocated_raiihandle(&ib_feed_resolutor, trimmed_ib_feed_cap, ib_feeder_mem.get())); 
                ...
                dg_sock::network_stack_allocation::NoExceptRawAllocation<char[]> ps_feeder_mem(ps_feeder_allocation_cost);
                auto ps_feeder                              = dg_sock::network_exception_handler::nothrow_log(dg_sock::network_producer_consumer::delvrsrv_open_preallocated_raiihandle(&ps_feed_resolutor, trimmed_ps_feed_cap, ps_feeder_mem.get()));  
                ...
                size_t gt_feeder_allocation_cost            = dg_sock::network_producer_consumer::delvrsrv_allocation_cost(&gt_feed_resolutor, trimmed_gt_feed_cap);
                dg_sock::network_stack_allocation::NoExceptRawAllocation<char[]> gt_feeder_mem(gt_feeder_allocation_cost);
                auto gt_feeder                              = dg_sock::network_exception_handler::nothrow_log(dg_sock::network_producer_consumer::delvrsrv_open_preallocated_raiihandle(&gt_feed_resolutor, trimmed_gt_feed_cap, gt_feeder_mem.get()));
                ...
                size_t bl_feeder_allocation_cost            = dg_sock::network_producer_consumer::delvrsrv_allocation_cost(&bl_feed_resolutor, trimmed_bl_feed_cap);
                dg_sock::network_stack_allocation::NoExceptRawAllocation<char[]> bl_feeder_mem(bl_feeder_allocation_cost);
                auto bl_feeder                              = dg_sock::network_exception_handler::nothrow_log(dg_sock::network_producer_consumer::delvrsrv_open_preallocated_raiihandle(&bl_feed_resolutor, trimmed_bl_feed_cap, bl_feeder_mem.get()));
                ...
                for (size_t i = 0u; i < consuming_sz; ++i){
                    ...
                    dg_sock::network_producer_consumer::delvrsrv_deliver(bl_feeder.get(), std::move(pkt.value()));
                }
        ...
        private:
            ...
            void recv(internal_segment_kernel_buffer * arr, size_t& sz, size_t cap) noexcept{
                ...
                dg_sock::network_stack_allocation::NoExceptAllocation<std::add_pointer_t<void>[]> addr_arr(cap);
                dg_sock::network_stack_allocation::NoExceptAllocation<size_t[]> capacity_arr(cap);
                dg_sock::network_stack_allocation::NoExceptAllocation<size_t[]> sz_arr(cap);
            ...
            struct InternalEntranceFeedResolutor: dg_sock::network_producer_consumer::ConsumerInterface<GlobalIdentifier>{
                ...
                void push(std::move_iterator<GlobalIdentifier *> id_arr, size_t sz) noexcept{
                    ...
                    for (size_t i = 0u; i < sz; ++i){
                        if (dg_sock::network_exception::is_failed(exception_arr[i])){
                            dg_sock::network_log_stackdump::critical(dg_sock::network_exception::verbose(dg_sock::network_exception::SOCKET_STREAM_LEAK));
                        }
                    }
                }
            };

            struct InternalInBoundFeedResolutor: dg_sock::network_producer_consumer::ConsumerInterface<internal_huge_kernel_buffer>{
            ...
            struct InternalPacketSegmentFeedResolutor: dg_sock::network_producer_consumer::ConsumerInterface<PacketSegment>{
                ...
                void push(std::move_iterator<PacketSegment *> data_arr, size_t sz) noexcept{
                    ...
                    dg_sock::network_stack_allocation::NoExceptAllocation<GlobalIdentifier[]> global_id_arr(sz);
                    dg_sock::network_stack_allocation::NoExceptAllocation<std::expected<AssembledPacket, exception_t>[]> assembled_arr(sz);
                    ...
                    for (size_t i = 0u; i < sz; ++i){
                        if (assembled_arr[i].has_value()){
                            ...
                            if (!buf.has_value()){
                                ...
                                dg_sock::network_producer_consumer::delvrsrv_deliver(this->inbound_feeder, std::move(buf.value()));
                            }
                        } else{
                            if (assembled_arr[i].error() != dg_sock::network_exception::SOCKET_STREAM_SEGMENT_FILLING){
                                dg_sock::network_log_stackdump::error_fast_optional(dg_sock::network_exception::verbose(assembled_arr[i].error()));
                            }
                        }
                    }
                }
            ...
            struct InternalInBoundGateFeedResolutor: dg_sock::network_producer_consumer::ConsumerInterface<PacketSegment>{

                InBoundGateInterface * inbound_gate;
                dg_sock::network_producer_consumer::DeliveryHandle<PacketSegment> * downstream_feeder;
                ...
                void push(std::move_iterator<PacketSegment *> data_arr, size_t sz) noexcept{
                    ...
                    for (size_t i = 0u; i < sz; ++i){
                        ...
                        dg_sock::network_producer_consumer::delvrsrv_deliver(this->downstream_feeder, std::move(base_data_arr[i]));
                    }
                }  
            };
    };
    ...
    struct ComponentFactory{
        ...
        static auto get_incremental_packet_id_generator(Address factory_addr) -> std::unique_ptr<PacketIDGeneratorInterface>{
            ...
            uint64_t random_counter = dg_sock::network_randomizer::randomize_int<uint64_t>();
        ...
        static auto get_random_hash_distributed_inbound_gate(std::vector<std::unique_ptr<InBoundGateInterface>> inbound_gate_vec,
            ...
            if (!stdxx::is_pow2(inbound_gate_vec.size())){
                dg_sock::network_exception::throw_exception(dg_sock::network_exception::INVALID_ARGUMENT);
            }
        ...
        static auto get_random_hash_distributed_blacklist_gate(std::vector<std::unique_ptr<BlackListGateInterface>> blacklist_gate_vec,
            ...
            if (!stdxx::is_pow2(blacklist_gate_vec.size())){
                dg_sock::network_exception::throw_exception(dg_sock::network_exception::INVALID_ARGUMENT);
            }
        ...
        static auto get_packet_assembler(size_t packet_map_cap,
            ...
            size_t max_consume_sz           = std::max(size_t{1}, tentative_max_consume_sz);
            size_t packet_map_bucket_cap    = dg_sock::cyclic_unordered_node_map<GlobalIdentifier, AssembledPacket>::ceil_size_to_capacity(packet_map_cap);
            auto packet_map                 = dg_sock::cyclic_unordered_node_map<GlobalIdentifier, AssembledPacket>(packet_map_bucket_cap);
        ...
        static auto get_buffer_fifo_container(size_t buffer_capacity,
            ...
            size_t upqueue_cap                  = stdxx::ceil2(buffer_capacity);
            ...
            return std::make_unique<BufferFIFOContainer>(dg_sock::pow2_cyclic_queue<BufferContainerWaitingItem>(stdxx::ulog2(stdxx::ceil2(push_concurrency_sz))),
                                                         dg_sock::pow2_cyclic_queue<internal_huge_kernel_buffer>(stdxx::ulog2(upqueue_cap)),
                                                         upqueue_cap,
                                                         stdxx::make_unique_fair_atomic_flag(),
        ...
        static auto get_fair_inbound_buffer_container(size_t distribution_queue_sz,
            ...
            return std::make_unique<FairInBoundBufferContainer>(dg_sock::pow2_cyclic_queue<FairBufferContainerWaitingItem>(stdxx::ulog2(stdxx::ceil2(push_concurrency_sz))),
                                                                dg_sock::pow2_cyclic_queue<dg_sock::vector<internal_huge_kernel_buffer>>(stdxx::ulog2(stdxx::ceil2(distribution_queue_sz))),
                                                                dg_sock::pow2_cyclic_queue<std::pair<std::optional<dg_sock::vector<internal_huge_kernel_buffer>> *, dg_binary_semaphore *>>(stdxx::ulog2(stdxx::ceil2(waiting_queue_sz))),
                                                                dg_sock::pow2_cyclic_queue<dg_sock::vector<internal_huge_kernel_buffer>>(stdxx::ulog2(stdxx::ceil2(leftover_queue_sz))),
                                                                get_default_lifo_unit_allocator<dg_sock::vector<internal_huge_kernel_buffer>>(queue_cap),
                                                                stdxx::make_unique_fair_atomic_flag(),
                                                                unit_sz);
        }
        ...
        static auto get_inbound_redistributor_worker(std::shared_ptr<InBoundContainerInterface> fr_warehouse,
                                                     ...
                                                     size_t fr_warehouse_get_cap,
                                                     size_t to_warehouse_push_cap,
                                                     size_t busy_threshold) -> std::unique_ptr<dg_sock::network_concurrency::WorkerInterface>{
        ...
        static auto get_expiry_worker(std::shared_ptr<PacketAssemblerInterface> packet_assembler,
                                      ...
                                      size_t busy_consumption_sz) -> std::unique_ptr<dg_sock::network_concurrency::WorkerInterface>{
        ...
        static auto get_flash_streamx_mailbox(std::vector<dg_sock::network_concurrency::daemon_raii_handle_t> daemon_vec,
                                              std::unique_ptr<PacketizerInterface> packetizer,
                                              std::shared_ptr<dg_sock::network_kernel_mailbox_impl1::core::MailboxInterface> base,
                                              std::shared_ptr<InBoundContainerInterface> inbound_container,
                                              std::shared_ptr<OutBoundRuleInterface> outbound_rule,
                                              size_t transmission_vectorization_sz) -> std::unique_ptr<dg_sock::network_kernel_mailbox_impl1::core::MailboxInterface>{
    ...
    struct Config{
        ...
        std::shared_ptr<OutBoundRuleInterface> outbound_rule;
        std::shared_ptr<dg_sock::network_concurrency_infretry_x::ExecutorInterface> infretry_device;
        std::shared_ptr<dg_sock::network_kernel_mailbox_impl1::core::MailboxInterface> base; //alright people might not like std::unique_ptr<> for throwing reasons, yet it is practice of extensibility to make this std::unique<>, we have to break practice for now
    };
