...
class TestWorker: public virtual dg_sock::network_concurrency::WorkerInterface
{
    ...
    private:
        ...
        void discrete_strong_send(dg_sock::network_kernel_mailbox_impl1::model::MailBoxArgument * arg_arr, size_t sz, size_t discrete_sz)
        {
        ...
        {
            static auto random_device       = std::bind(std::uniform_int_distribution<size_t>{}, std::mt19937_64{static_cast<size_t>(std::chrono::high_resolution_clock::now().time_since_epoch().count())});
            const size_t PACKET_COUNT_RANGE = size_t{1} << 8;
...
{
    {
...

        {
            ...
            dg_sock::network_concurrency::daemon_register(dg_sock::network_concurrency::MAILBOX_UNIT_DAEMON, std::move(test_worker));
        ...
        dg_sock::network_concurrency::deinit();
    }
}
