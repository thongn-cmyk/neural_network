...
namespace dg_sock::network_hash_factory
{
    template <class T>
    ...
    template <class T, std::enable_if_t<dg_sock::network_trivial_serializer::is_serializable_v<T>, bool> = true>
    struct trivial_reflectable_hasher{

        constexpr auto operator()(const T& value) const noexcept -> size_t{

            constexpr size_t SZ = dg_sock::network_trivial_serializer::size(T{});  
            std::array<char, SZ> buf = {};
            dg_sock::network_trivial_serializer::serialize_into(buf.data(), value);

            return dg_sock::network_hash::hash_bytes(buf.data(), std::integral_constant<size_t, SZ>{});
        }
    };

    template <class T>
    static inline constexpr bool is_trivial_hashable_v = dg_sock::network_trivial_serializer::is_serializable_v<T>;
    ...
    template <class T>
    static inline constexpr bool is_trivial_equal_to_able_v = dg_sock::network_trivial_serializer::is_serializable_v<T>;
