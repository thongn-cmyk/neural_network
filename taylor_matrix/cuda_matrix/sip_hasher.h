#ifndef __TAYLOR_MATRIX_CUDA_MATRIX_SIP_HASHER_H__
#define __TAYLOR_MATRIX_CUDA_MATRIX_SIP_HASHER_H__
...
namespace taylor_matrix::cuda_matrix::sip_hasher
{
    ...
    class SipHasher
    {
        ...
        private:
            ...
            __device__ static constexpr auto rotl(uint64_t x, int b) -> uint64_t
            {
                return (x << b) | (x >> (64 - b));
            }
        ...
        public:
            ...
            __device__ explicit constexpr SipHasher(std::array<char, 16u> key): m(0u),
                                                                                m_sz(0u)
            {
                uint64_t k0 = 0u;
                uint64_t k1 = 0u;
            ...
            template <class T>
            __device__ constexpr SipHasher(implicit_key_tag,
                                           const T& key): SipHasher(std::bit_cast<std::array<char, 16u>>(key)){}

            template <size_t SZ>
            __device__ constexpr void update(const char * data, const std::integral_constant<size_t, SZ>)
            {
            ...
            __device__ constexpr void update(const char * data, size_t sz)
            {
                for (size_t i = 0u; i < sz; ++i)
                {
                    this->m     |= static_cast<uint64_t>(std::bit_cast<uint8_t>(data[i])) << (this->m_sz * 8);
            ...
            __device__ constexpr auto get_hash() -> uint64_t
            {
        ...
        private:
            ...
            __device__ constexpr auto internal_finalize() -> uint64_t
            {
            ...
            __device__ constexpr void sipround()
            {
                this->v0    += this->v1;
                this->v1    = rotl(v1, 13);
                this->v1    ^= this->v0;
            ...
            __device__ constexpr void process_block(uint64_t block)
            {
