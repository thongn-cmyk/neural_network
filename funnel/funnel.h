...
namespace funnel{
    ...
    auto delvrsrv_open_preallocated_handle(ConsumerInterface<event_t> * consumer, size_t deliverable_cap, char * preallocated_buf) noexcept -> std::expected<DeliveryHandle<event_t> *, exception_t>{
        ...
        try{
            auto raii_event_ptr     = funnel::raii_inplace_construct<event_t[]>(preallocated_buf, deliverable_cap);
            size_t container_sz     = 0u;
            size_t container_cap    = deliverable_cap;
            auto raii_handle_ptr    = funnel::raii_inplace_construct<DeliveryHandle<event_t>>(std::next(preallocated_buf, funnel::inplace_construct_size<event_t[]>(deliverable_cap)),
    ...
    auto delvrsrv_allocation_cost(ConsumerInterface<event_t> * consumer, size_t deliverable_cap) noexcept -> size_t{
        ...
        return funnel::inplace_construct_size<event_t[]>(deliverable_cap) 
               + funnel::inplace_construct_size<DeliveryHandle<event_t>>();
    }
    ...
    auto delvrsrv_close_preallocated_handle(DeliveryHandle<event_t> * handle) noexcept{
        ...
        funnel::inplace_destruct_array(handle->deliverable_arr, handle->deliverable_cap);
        funnel::inplace_destruct_object(handle);
    }
    ...
    auto bump_allocator_preallocated_initialize(size_t buf_sz, char * buf) -> std::expected<BumpAllocatorResource *, exception_t>{
        ...
        return funnel::inplace_construct<BumpAllocatorResource>(std::next(buf, buf_sz), resource);
    }
    ...
    void bump_allocator_preallocated_deinitialize(BumpAllocatorResource * obj) noexcept{
        ...
        funnel::inplace_destruct_object(stdx::safe_ptr_access(obj));
    }    
        ...
        public:
            ...
            auto allocate(std::size_t n) -> T *
            {
                ...
                else
                {
                    ...
                    raw_buf                 = stdx::align_ptr(std::add_pointer_t<char>(char_reference), std::integral_constant<size_t, alignof(T)>{});
    ...
    using kv_feed_unordered_map_t = datastructure::unordered_map_variants::unordered_node_map<KeyType,
                                                                                              MappedType,
                                                                                              std::size_t,
                                                                                              std::integral_constant<bool, true>,
                                                                                              hasher::default_hasher<KeyType>,
                                                                                              hasher::default_equal_to<KeyType>,
    ...
    void delvrsrv_kv_destroy_event_container(KVEventContainer<EventType> event_container) noexcept{
        ...
        funnel::inplace_destruct_array(event_container.ptr, event_container.cap);
    }
    ...
    auto delvrsrv_kv_open_handle(KVConsumerInterface<key_t, event_t> * consumer, size_t deliverable_cap) noexcept -> std::expected<KVDeliveryHandle<key_t, event_t> *, exception_t>{
        ...
        try{
            ...
            common_exception::remove_expected(bump_allocator_align(bump_allocator.value(), alignof(event_t)));
    ...
    auto delvrsrv_kv_open_preallocated_handle(KVConsumerInterface<key_t, event_t> * consumer,
        ...
        try
        {
...

            {
                ...
                common_exception::dg_nothrow(bump_allocator_align(bump_allocator, alignof(event_t)));
    ...
    void delvrsrv_kv_clear(KVDeliveryHandle<key_t, event_t> * handle) noexcept{
        ...
        common_exception::dg_nothrow(bump_allocator_align(handle->bump_allocator, alignof(event_t)));
    ...
    __attribute__((noinline)) void delvrsrv_kv_deliver_slow_path(KVDeliveryHandle<key_t, event_t> * handle, const key_t& key, event_t event) noexcept{
        ...
        if (map_ptr == handle->key_event_map.end()){
            ...
            if (!buf.has_value()) [[unlikely]]{
                if (buf.error() == common_exception::RESOURCE_EXHAUSTION){
                    delvrsrv_kv_clear(handle);
                    buf = common_exception::remove_expected(bump_allocator_allocate(handle->bump_allocator, delvrsrv_kv_get_event_container_bsize<event_t>(DELVRSRV_KV_EVENT_CONTAINER_INITIAL_CAP)));
                } else{
            ...
            try{
                ...
                delvrsrv_kv_clear(handle);
                char * nofail_buf                   = common_exception::remove_expected(bump_allocator_allocate(handle->bump_allocator, delvrsrv_kv_get_event_container_bsize<event_t>(DELVRSRV_KV_EVENT_CONTAINER_INITIAL_CAP)));
        ...
        common_exception::dg_nothrow(delvrsrv_kv_push_event_container(map_ptr->second, std::move(event)));
