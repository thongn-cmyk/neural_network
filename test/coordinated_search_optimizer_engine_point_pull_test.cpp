...
auto get_random_point_bag(size_t sz) -> std::vector<std::pair<tensor_std_float_t, tensor_std_float_t>>
{
    ...
    {
        rs.push_back(std::make_pair(randomize_double(0, 10), randomize_double(0, 1)));
    }
...
//I think our engine is ready
//I've been testing it very thoroughly and I dont think that it can be better optimized, actually...
//until the deviation is +/- 0.1% for the first step, I dont think this is good enough
...
void run_one_test()
{
    ...
    size_t point_pull_sz    = 8u;
    size_t coefficient_sz   = 20u;
    ...
    {
        auto other_expected_matrix  = test_engine->optimize(*expected_matrix, *evaluator, cancellation_token);
        double optimized_deviation  = evaluator->get_deviation(*other_expected_matrix);
