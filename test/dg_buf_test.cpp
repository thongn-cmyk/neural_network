...
{
    using namespace dg::dgbuf::types_space;
...
void test_one_dg_buf()
{
    ...
    auto callback   = []<class T>(T)
    {
        ...
        std::string buf             = {};
        auto tmp_serialized_obj     = dg::dgbuf::stl_to_dgbuf::serializer{}.serialize(obj, buf);
        
        auto serialized_obj         = dg::dgbuf::autosetter::AutoSetter{}.auto_set(tmp_serialized_obj, buf.data());
        auto other_serialized_obj   = serialized_obj;

        auto deserialized_obj       = dg::dgbuf::dgbuf_to_stl::deserializer{}.deserialize(serialized_obj);

        if (serialized_obj != other_serialized_obj)
        {
            std::cout << "mayday, mismatch representation 0" << std::endl;
            std::abort();
        }
        ...
        {
            std::cout << "mayday, mismatch representation 1" << std::endl;
            std::abort();
        }
