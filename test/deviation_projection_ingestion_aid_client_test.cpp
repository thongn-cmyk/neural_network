...
void test_ingestion_aid()
{
    ...
    deviation_projection_ingestion_aid_client::APIClient ingestion_aid(get_local_remote());
    std::string fp = randomize_hex_token_file();

    ingestion_aid.run(get_random_ingestion_aid_run_payload(fp, client.get_client_remote()))->wait();
    wait_client(&ingestion_aid);
