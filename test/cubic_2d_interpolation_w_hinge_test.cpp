...
#include <taylor_matrix/host_matrix/two_dimensional_cubic_interpolation.h>

using tensor_std_float_t = tensor_model::tensor_std_float_t;

using namespace float_def;

//ok, we are going to test 2d -> 1d projections
//ok interpolation works

//we'd do uniform range interpolation later
//interpolation is probably one of the most important definition in continuous projection, for the reason being that we are on the same continuous string to do DP (dynamic programming)

//this is where things get very hazy, because originally, we are on 1 dimension, (a * 256 + b) * 256 + c, we can still do domain interpolation to approx, if we are to approach x -> 0 for x is the distance to the next neighbor
//but at the base of interpolation, we leverage a straight line, polynomial to compress, such is impossible without the bigbang, or SVD, single value decomposition

//so there are two distinct definitions, one cannot be without another

//there are no other answers, I can guarantee you that
...
auto get_random_point_bag(size_t sz) -> std::vector<std::pair<std::pair<tensor_std_float_t, tensor_std_float_t>, tensor_std_float_t>>
{
    return
    {
        {{0, 0},        1},
...
class Projector
{
    ...
    public:
        ...
        auto project(float x0, float x1)
        {
...
class CubicSplinePointPullMatrixEvaluator: public virtual matrix_evaluator::MatrixEvaluatorInterface
{
    ...
    public:
        ...
        CubicSplinePointPullMatrixEvaluator(std::shared_ptr<Projector> spline_projector,
                                            std::vector<std::pair<std::pair<tensor_std_float_t, tensor_std_float_t>, tensor_std_float_t>> point_vec): spline_projector(std::move(spline_projector)),
                                                                                                                                                      point_vec(std::move(point_vec)){}

        auto get_deviation(the_matrix::MatrixInterface& matrix) -> eval_float_t
        {
            std::vector<std::pair<std::pair<tensor_std_float_t, tensor_std_float_t>, tensor_std_float_t>> expected_vec{};
            ...
            {
                float actual_y  = this->spline_projector->project(x.first, x.second);
    ...
    private:
                              ...
                              const std::vector<std::pair<std::pair<tensor_std_float_t, tensor_std_float_t>, tensor_std_float_t>>& rhs) -> eval_float_t
        {
            ...
            {
                rs += std::pow(lhs[i].first.first - rhs[i].first.first, 2);
                rs += std::pow(lhs[i].first.second - rhs[i].first.second, 2);
...
auto get_distributed_matrix_evaluator(const std::vector<std::pair<std::pair<tensor_std_float_t, tensor_std_float_t>, tensor_std_float_t>>& point_vec,
                  ...
                  const size_t hash_table_sz)
{
    ...
    std::shared_ptr<Projector> spline_projector                                                 = get_uniform_spline_projector();
    ...
    std::vector<std::pair<std::pair<tensor_std_float_t, tensor_std_float_t>, tensor_std_float_t>> point_bag = get_random_point_bag(point_pull_sz);
