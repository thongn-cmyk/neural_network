...
#include <matrix_steering_subsystem/global_optimality_approximator.h>
#include <stdint.h>
#include <stdlib.h>
#include <memory>
...
auto randomize_root_vec_2() -> std::vector<double>
{
    ...
    for (size_t i = 0u; i < root_sz; ++i)
    {
        result.push_back(randomize_real() + 100000ULL);
    }
...
auto run_one_root_test() -> double
{
    ...
    if (std::abs(actual_root) < EPSILON)
    {
        if (std::abs(optimized_root_x.value()) < EPSILON)
        {
            return 1;
        }
    }
