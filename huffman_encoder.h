...
namespace dg::network_huffman_encoder::constants{
    ...
    //assume that our tree height is 32
    //I forgot the proof that I once proved
    //then the tree population must be at least 2 ** 32 or somewhere around that

    //assume the worst case scenerio of tree, we'd have a one left, all right, recursively built tree
    //or one right all left recursive built tree
...
namespace dg::network_huffman_encoder::serialization{
    ...
    static auto serialize_model(const SerializableModel& model) -> std::string{
        ...
        std::vector<bool> rem(std::next(model.header.begin(), bitarr_bitlength), model.header.end());
        std::vector<char> promoted_rem{};

        for (bool c: rem)
        {
            promoted_rem.push_back(std::bit_cast<char>(c));
        }

        auto serializable = std::make_tuple(std::move(bitarr), std::move(promoted_rem), model.c_vec); 
    ...
    static auto deserialize_model(const std::string& bstream) -> SerializableModel{
        ...
        SerializableModel rs{};
        std::tuple<std::vector<uint32_t>, std::vector<char>, std::vector<model::word_type>> serializable{};
        ...
        for (char c: rem)
        {
            rs.header.push_back(std::bit_cast<bool>(c));
        }
...
namespace dg::network_huffman_encoder{
    ...
    auto encode(const std::basic_string<char, Args...>& inp) -> std::basic_string<char, Args...>{
        ...
        std::string model_stream            = serialize_model(model.get());
