...
{
    static inline constexpr size_t EXPBACKOFF_MUTEX_LOCK_SPIN_SZ            = size_t{1} << 4;
    static inline constexpr size_t EXPBACKOFF_FAIR_AF_YIELD_LOCK_SPIN_SIZE  = size_t{1} << 10;
    ...
    struct fair_atomic_flag
    {
        ...
        std::atomic<std::thread::id> prev_thr_id;
    ...
    auto make_fair_atomic_flag(bool value = false) noexcept -> fair_atomic_flag
    {
        return fair_atomic_flag
        {
            ...
            .prev_thr_id        = std::atomic<std::thread::id>(NULL_THREAD_ID),
    ...
    inline bool eventloop_expbackoff_spin(Lambda&& lambda, 
                                          size_t spin_sz) noexcept(noexcept(lambda()))
    {
    ...
    {
        if (std::this_thread::get_id() == NULL_THREAD_ID) [[unlikely]]
        {
            std::abort();
        }
        ...
        eventloop_expbackoff_spin(yield_job, EXPBACKOFF_FAIR_AF_YIELD_LOCK_SPIN_SIZE);
        ...
        if (busy_waiter_sz == 0u)
        {
            if (!job())
            {
                while (true)
                {
                    if (eventloop_expbackoff_spin(job, EXPBACKOFF_MUTEX_LOCK_SPIN_SZ))
                    {
                        break;
                    }
    ...
    inline __attribute__((always_inline)) void fair_atomic_flag_memsafe_lock(fair_atomic_flag * mtx)
    {
        ...
        if (mtx->prev_thr_id.exchange(std::this_thread::get_id(), std::memory_order_relaxed) == std::this_thread::get_id())
        {
            return;
        }
